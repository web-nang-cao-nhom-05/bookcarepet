...
class Tin_tuc extends CI_Controller {
	...
	public function hien_thi_tin_tuc_chi_tiet()
    {
		...
		// Kết nối đến MODEL
		$this->load->model('m_tin_tuc');
    	$ma_tin_tuc = $this->uri->segment(3);
    	$data['tin_tuc_chi_tiet'] = $this->m_tin_tuc->lay_tin_tuc_theo_ID($ma_tin_tuc);
    	$query = $this->db->query("
			SELECT ma_tin_tuc, anh_minh_hoa, tieu_de, date(thoi_gian) as thoi_gian
			FROM tin_tuc
			ORDER BY thoi_gian DESC limit 3
